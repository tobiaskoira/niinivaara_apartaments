...
<script>
  function initCalendar() {
    if (!window.FullCalendar) {
      return setTimeout(initCalendar, 100);
    }
    ...
    for (let i = 0; i < 90; i++) {
      ...
      events.push({
        title: isBusy ? 'Varattu' : 'Vapaa',
        start: dateStr,
        allDay: true,
        backgroundColor: isBusy ? 'red' : 'green',
        borderColor: isBusy ? 'red' : 'green',
        textColor: 'white',
        extendedProps: {
          isFree: !isBusy
        }
    ...
    const calendar = new FullCalendar.Calendar(calendarEl, {
      ...
      eventClick: function(info) {
        ...
        if (event.extendedProps.isFree) {
          selectedDateEl.textContent = event.startStr;
