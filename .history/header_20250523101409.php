...
<a href="<?php echo esc_url(home_url('/')); ?>" class="inline-block">
  <img src="<?php echo get_template_directory_uri(); ?>/assets/images/logo.svg" alt="Site Logo" class="h-8 w-auto sm:h-16 md:h-20 lg:h-24 ">
    ...
    <div class="sm:hidden flex flex-1 items-center justify-center" id="hamburger-icon">
      <button class="text-xl text-white" id="hamburger-menu">
        <span class="block w-6 h-2 bg-[var(--main-color)] mb-2"></span>
        <span class="block w-6 h-2  bg-[var(--main-color)] mb-2"></span>
        <span class="block w-6 h-2  bg-[var(--main-color)]"></span>
      </button>
    </div>
  <!-- Regular Menu -->
  <nav class="sm:flex-1 sm:flex sm:justify-center hidden " id="menu-items">
    <?php
      wp_nav_menu([
        ...
        'container' => false,
        'menu_class' => 'flex gap-6 text-lg uppercase  text-[var(--main-color)]',
  ...
  <nav class="bg-white-500 flex-row items-center justify-center w-full hidden transition-all duration-300 ease-in-out " id="menu-items-mobile">
    <?php
      wp_nav_menu([
        'theme_location' => 'primary',
        'container' => false,
        'menu_class' => 'flex flex-col gap-6 text-lg font-semibold uppercase  text-[var(--main-color)] bg-white',
        'items_wrap' => '<ul id="%1$s" class="%2$s">%3$s</ul>',
      ]);
    ?>
  </nav>
