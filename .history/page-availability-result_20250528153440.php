...
if ($arrival && $departure) {
    ...
    // Save formatted strings BEFORE the loop modifies the object
    $formattedArrival = $start->format('d.m.Y');
    $formattedDeparture = (new DateTime($departure))->format('d.m.Y');
...
echo '<h2 class="text-2xl font-bold text-[var(--main-color)]">Huoneistot on saatavilla ajalla ' . $formattedArrival . ' – ' . $formattedDeparture . '!</h2>';

echo '<div class="grid grid-cols-2 gap-4">';
echo '<div class="bg-gray-100 p-4">Column 1 content</div>';        
echo '<div class="bg-gray-100 p-4">Column 2 content</div>';       
echo '</div>';

   

    echo "<p class='mt-2 text-lg'>Täytä lomake äläpuolella varataksesi.</p>";
    echo do_shortcode('[contact-form-7 id="7a91262" title="Booking form"]');
} else {
