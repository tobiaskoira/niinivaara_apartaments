...
<img src="<?php echo get_template_directory_uri(); ?>/assets/images/header.jpg"
  ...
  <nav class="sm:flex-1 sm:flex sm:justify-center hidden  " id="menu-items">
    <?php
      wp_nav_menu([
        ...
        'container' => false,
        'menu_class' => 'flex gap-6 text-base md:text-lg lg:text-xl xl:text-3xl font-semibold uppercase  text-[var(--main-color)] xl:text-[var(--main-light)]',
  ...
  <div class="flex gap-2 text-1xl font-medium text-[var(--main-light)]">
  ...
  <nav id="menu-items-mobile"
      ...
      class="absolute top-0 -top-full left-0 w-full bg-[var(--button-background)] z-50 transition-all duration-300 ease-in-out">
  <button id="close-mobile-menu" class="self-end p-4 text-3xl text-[var(--main-color)]">
    ...
    <?php
      wp_nav_menu([
        'theme_location' => 'primary',
        'container' => false,
        'menu_class' => 'flex flex-col gap-6 text-1xl font-semibold uppercase text-[var(--main-color)] p-6',
        'items_wrap' => '<ul id="%1$s" class="%2$s">%3$s</ul>',
      ]);
    ?>
  </nav>
  ...
  <div class="flex flex-1 items-center justify-center pt-24">
    ...
    <div class="text-[var(--main-color)] text-center p-6 rounded z-10 relative">
      <h1 class="text-4xl md:text-5xl text-[var(--main-main)] sm:text-[var(--main-light)] font-bold drop-shadow">Varaa huoneistot</h1>

    <?php
    // Make sure shortcodes are available
    if (shortcode_exists('calendar_booking_form')) {
        echo do_shortcode('[calendar_booking_form]');
    } else {
        echo '<!-- Shortcode not available yet -->';
    }
    ?>
    </div>
