...
add_filter('nav_menu_css_class', function ($classes, $item) {
    // Remove "current-menu-item" from anchor links on front page
    if (is_front_page() && isset($item->url) && strpos($item->url, '#') !== false) {
        $classes = array_diff($classes, ['current-menu-item']);
    }

    // Optionally: keep it only for the first one
    static $has_set_one = false;
    if (!$has_set_one && is_front_page() && strpos($item->url, '#') !== false) {
