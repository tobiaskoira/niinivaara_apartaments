...
add_shortcode('calendar_booking_form', function () {
    ...
    $busyJson = json_encode($busyDates);
$action_url = site_url('/availability-result/');
    return <<<HTML
<div class="max-w-md mx-auto  text-white rounded-xl p-6 mt-6">
  ...
  <form method="GET" action="<?php echo site_url('/availability-result/'); ?>" class="space-y-4">
    <div class=" flex sm:flex-nowrap flex-wrap gap-4 items-stretch justify-center">
      <input type="date" id="arrival"  name="arrival" class="flex-1 min-w-[120px] px-4 py-2 rounded text-black" required>
      <input type="date" id="departure" name="departure" class="flex-1 min-w-[120px] px-4 py-2 rounded text-black" required>
    ...
    <button type="submit" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded flex-1 min-w-[120px]">
      Tarkista saatavuus
    </button>  
