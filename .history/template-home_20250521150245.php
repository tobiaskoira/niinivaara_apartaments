...
<div class="max-w-6xl mx-auto p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
  <!-- Левая колонка с одной картинкой -->
  <div>
    <?php echo wp_get_attachment_image($gallery_image_ids[0], 'large', false, ['class' => 'w-full h-full object-cover rounded-lg transform transition-transform duration-300 hover:scale-105']); ?>
  </div>
  ...
  <div class="grid grid-rows-2 gap-4">
    <?php
    for ($i = 1; $i <= 2; $i++) {
      echo wp_get_attachment_image($gallery_image_ids[$i], 'medium', false, ['class' => 'w-full h-full object-cover rounded-lg transform transition-transform duration-300 hover:scale-105']);
    }
    ?>
  </div>

  <!-- Третья колонка с двумя картинками -->
  <div class="grid grid-rows-2 gap-4">
    <?php
    for ($i = 3; $i <= 4; $i++) {
      echo wp_get_attachment_image($gallery_image_ids[$i], 'medium', false, ['class' => 'w-full h-full object-cover rounded-lg transform transition-transform duration-300 hover:scale-105']);
